...
carnatic-log/
  ...
  scripts/
    ...
    augmented_keys.py                 # Backfill ragam_key/composer_key on songs, key on ragams
    build_composers.py          # Derive composers.json from songs.json (merge-safe)
    augmented_discover.py             # Stage 1: URL discovery (blogs, sitemaps, Wikipedia)
    augmented_fetch.py                # Stage 2: Polite fetch + content extraction, checkpointed
    augmented_structure.py            # Stage 3: Gemini page structuring, entity linking -> augmentations.json
    augmented_synthesize.py           # Stage 4: Per-entity summary synthesis -> ragams/composers
...
Generated by `scripts/augmented_keys.py`.
...
- **`summary`**: Curated <=6-sentence summary (populated by `augmented_synthesize.py`).
...
|----------|--------|---------|
| `normalize_ragam(ragam)` | `augmented_keys.py`, `build_composers.py` | Deterministic ragam key: lowercase, collapse digraphs, strip non-alnum |
| `composer_slug(composer)` | `augmented_keys.py`, `build_composers.py` | Deterministic composer key: canonicalize via `COMPOSER_CANONICAL`, then slugify |
...
### 11.1 Stage 1: URL Discovery (`augmented_discover.py`)
...
```bash
uv run scripts/augmented_discover.py
```

### 11.2 Stage 2: Polite Fetch (`augmented_fetch.py`)
...
```bash
uv run scripts/augmented_fetch.py
```

### 11.3 Stage 3: Structure and Link (`augmented_structure.py`)
...
```bash
uv run scripts/augmented_structure.py
```

### 11.4 Stage 4: Synthesize Summaries (`augmented_synthesize.py`)
...
```bash
uv run scripts/augmented_synthesize.py
```
...
uv run scripts/augmented_keys.py
...
| File | Purpose |
|------|---------|
| `scripts/augmented_keys.py` | Backfill `ragam_key` and `composer_key` onto songs.json, `key` onto ragams.json |
| `scripts/build_composers.py` | Derive composers.json from songs.json (merge-safe, preserves enrichment fields) |
| `scripts/augmented_discover.py` | Stage 1: URL discovery from blog indices, WordPress sitemaps, Wikipedia API |
| `scripts/augmented_fetch.py` | Stage 2: Polite fetch with content extraction and content_hash checkpointing |
| `scripts/augmented_structure.py` | Stage 3: Gemini-powered page structuring, entity linking, builds augmentations.json |
| `scripts/augmented_synthesize.py` | Stage 4: Per-entity summary synthesis from page summaries |
